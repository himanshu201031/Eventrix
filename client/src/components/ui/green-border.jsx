...
/**
 * ShaderBackground — WebGL "pulsing border" shader, adapted from Paper Shaders
 * (https://shaders.paper.design/pulsing-border, Apache-2.0). Zero runtime
 * dependencies: one WebGL canvas that fills its parent. Drop it behind content
 * with <div className="relative"><ShaderBackground className="absolute inset-0" />
 * …</div>
 *
 * Eventrix improvements over the source:
 *  - Palette is now the Eventrix brand (purple → pink → orange → lime) and can
 *    be overridden with hex strings via the `palette` prop.
 *  - Uniforms are exposed as optional props instead of module constants.
 *  - prefers-reduced-motion freezes the animation to a static frame.
 *  - webglcontextlost pauses rendering instead of silently breaking the loop.
 *  - Canvas is aria-hidden (pure decoration).
 */
...
/** Eventrix brand ramp: dark base → purple → pink → orange → lime. */
const DEFAULT_PALETTE = ['#0d0d11', '#ba28e2', '#ff2d7a', '#ff5a1f', '#a6ff00'];
 ...
 * @param {string[]} [props.palette] Brand hex colors, floor → tip of the ramp.
 *        Defaults to the Eventrix purple/pink/orange/lime ramp.
 * @param {number} [props.timeScale] Animation speed; 0 freezes the frame.
 * @param {number} [props.intensity] Pulse frequency along the border.
 * @param {number} [props.paramA] Border thickness (0 = hairline, 1 = thick).
 * @param {number} [props.hue] Hue rotation in radians (0 keeps the palette true).
 * @param {number} [props.grain] Film-grain amount (0 = clean).
 * @param {number} [props.vignette] Corner darkening (0 = none).
 * @param {boolean} [props.cursorEnabled] Let the pointer distort the border.
 */
export function ShaderBackground({
    ...
    // Frozen on first render: consumers pass static tuning, and the WebGL
    // program is built once. Prefer remounting (React key) to re-tune live.
    const settingsRef = useRef(null);
    if (settingsRef.current === null) {
        const colors = palette ? palette.map(hexToRgb01) : DEFAULT_PALETTE.map(hexToRgb01);
        const padded = [...colors];
        while (padded.length < 8) padded.push(colors[colors.length - 1]);
        settingsRef.current = {
            ...DEFAULTS,
            colors: padded.slice(0, 8),
            colorCount: palette ? palette.length : DEFAULT_PALETTE.length,
            timeScale: timeScale ?? DEFAULTS.timeScale,
            intensity: intensity ?? DEFAULTS.intensity,
            paramA: paramA ?? DEFAULTS.paramA,
            hue: hue ?? DEFAULTS.hue,
            grain: grain ?? DEFAULTS.grain,
            vignette: vignette ?? DEFAULTS.vignette,
            cursorEnabled: cursorEnabled ?? DEFAULTS.cursorEnabled,
        };
    }
    ...
    useEffect(() => {
        ...
        // Reduced motion freezes the animation to a single static frame.
        const reduceMotion =
            typeof window.matchMedia === 'function' &&
            window.matchMedia('(prefers-reduced-motion: reduce)').matches;
        const timeScale = reduceMotion ? 0 : settings.timeScale;
        ...
        gl.uniform4f(uni.cursor, 0, settings.cursorEffect, settings.cursorStrength, settings.cursorRadius);
        ...
        function render(now) {
            ...
            gl.uniform4f(uni.scene, width, height, ((now - start) / 1000) * timeScale, settings.colorCount);
            gl.uniform4f(uni.space, settings.offsetX, settings.offsetY, mouseX, mouseY);
            gl.uniform4f(
                uni.cursor,
                settings.cursorEnabled ? cursorPresence : 0,
